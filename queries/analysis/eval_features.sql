...
,case
    ...
    when left(column_name, 7) in ('n_total','n_disti','n_days_','max_cli','n_revis','n_inter','mean_pr','max_rev') then 1
    else 0 end is_engagement
,case
    when column_name in ('is_stem','course_level','studied_credits','num_of_prev_attempts','reg_date') then 1
    when left(column_name, 9) in ('submitted','avg_score','studied_c','sum_score','sum_weigh','course_we','n_assessm') then 1
...
,case
    ...
    when left(column_name, 6) in ('n_expe','n_weig','n_unwe','max_click','is_0_1','is_imd','is_age','is_hig','n_asses','n_inter','') then 1
    else 0 end is_demographics
