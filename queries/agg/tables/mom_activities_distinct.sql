...
select
 v.student_id
,count(*)                      n
,cast(sum(v.n_activity_types) 
    as float) / count(*)       n_activity_types
,cast(sum(v.n_activities) 
    as float) / count(*)       n_activities
...
,|/(sum((v.n_activity_types - avg.n_activity_types)^2) / avg.n)     stddev_n_activity_types

,avg.n_activities                     avg_n_activities
,sum((v.n_activities - avg.n_activities)^2) / avg.n var_n_activities
,|/(sum((v.n_activities - avg.n_activities)^2) / avg.n) stddev_n_activities

into first30.mom_activities_12
from agg.v_student_activities_per_day v
