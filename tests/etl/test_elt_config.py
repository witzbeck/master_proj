...
def test_get_all_table_names_with_schema(
    ...
    assert all(isinstance(x, str) for x in all_table_names_with_schema), (
        f"{all_table_names_with_schema}"
    )
    assert all("." in x for x in all_table_names_with_schema), (
        f"{all_table_names_with_schema}"
    )
...
def test_data_dir_source_path_dict(data_dir: DataDirectory) -> None:
    ...
    assert all(isinstance(x, Path) for x in data_dir.source_path_dict.values()), (
        f"{data_dir.source_path_dict}"
    )
    assert all(x.is_file() for x in data_dir.source_path_dict.values()), (
        f"{data_dir.source_path_dict}"
    )
...
def test_data_dir_staging_path_dict(data_dir: DataDirectory) -> None:
    ...
    assert all(isinstance(x, Path) for x in data_dir.staging_path_dict.values()), (
...
def test_queries_dir_schema_dict_has_all_subdirs(queries_dir: QueriesDirectory) -> None:
    assert all(isinstance(x, Directory) for x in queries_dir.schema_dict.values()), (
...
def test_queries_dir_schema_dict_has_valid_schema_names(
    ...
    assert all(not x[0].isnumeric() for x in queries_dir.schema_dict.keys()), (
        f"{queries_dir.schema_dict}"
    )
...
def test_queries_dir_schema_dict_has_only_sql_files(
    ...
    assert all(x.istype(".sql") for x in queries_dir.landing_dir.filelist), (
...
def test_sorted_nonlanding_query_paths_is_paths(
    ...
    assert all(isinstance(x, Path) for x in sorted_nonlanding_query_paths), (
...
def test_sorted_nonlanding_query_paths_is_sql(
    ...
    assert all(x.suffix == ".sql" for x in sorted_nonlanding_query_paths), (
        f"{sorted_nonlanding_query_paths}"
    )
...
def test_sorted_nonlanding_query_paths_is_nonlanding(
    ...
    assert all("00_landing" not in x.parts for x in sorted_nonlanding_query_paths), (
        f"{sorted_nonlanding_query_paths}"
    )
...
def test_staging_path_groups_is_objects(
    ...
    assert all(isinstance(x, StagingPathGroup) for x in staging_path_groups), (
