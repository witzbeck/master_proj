...
from etl import queries_dir
from etl.utils import check_sql_path_for_blacklist


@fixture(
    scope="module",
    params=[x for x in queries_dir.allchildfiles if x.path.suffix == ".sql"],
    ids=[
        "/".join(x.path.parts[-3:])
        for x in queries_dir.allchildfiles
...
@fixture(
    scope="module",
    params=queries_dir.allchilddirs,
    ids=["/".join(x.path.parts[-2:]) for x in queries_dir.allchilddirs],
)
