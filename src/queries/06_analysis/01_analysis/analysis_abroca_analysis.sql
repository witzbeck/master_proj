...
from (
        select row_number() over (
                partition by v.model_type_id,
                a.course_id --,a.run_id
                --,a.iter_id   
                order by v.model_type_id,
                    a.course_id --,a.run_id
                    --,a.iter_id   
            ) n,
            a.course_id,
            ...
            r.disabled_ratio,
            w.warnings
        from analysis.abroca a
            join analysis.all_runs_results v on v.run_id = a.run_id
            and v.iter_id = a.iter_id
            join first30.all_classes_ratios r on r.course_id = a.course_id
            left join model.warnings w on w.run_id = a.run_id
    ) a
