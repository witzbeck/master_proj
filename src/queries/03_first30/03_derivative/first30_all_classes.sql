select a.id student_id,
    ...
    a.final_result_id,
    case
        when vf.is_distinction + vf.is_pass > 0 then 1
        else 0
    end is_pass_or_distinction,
    case
        when vf.is_fail + vf.is_withdrawn > 0 then 1
        else 0
    end is_withdraw_or_fail,
    vf.is_distinction,
    vf.is_pass,
    vf.is_fail,
    vf.is_withdrawn,
...
from main.student_info a
    ...
    left join main.onehot_imd_band vi on vi.id = a.imd_band_id
    left join main.onehot_region vr on vr.id = a.region_id