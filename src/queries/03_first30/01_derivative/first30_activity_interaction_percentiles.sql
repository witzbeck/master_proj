with freq as (
    ...
    from main.student_vle_bridge b
        ...
        join first30.student_ids s on s.id = b.student_id
...
select f.course_id,
    ...
    ntile(100) over (
        order by f.n_distinct_visits
    ) distinct_visits_percentile,
    ntile(100) over (
        order by f.n_total_visits
    ) total_visits_percentile,
    ntile(100) over (
        order by f.n_total_clicks
    ) total_clicks_percentile,
    ntile(100) over (
        order by f.avg_clicks_per_day
    ) avg_clicks_per_day_percentile,
    ntile(100) over (
        order by f.avg_clicks_per_visit
    ) avg_clicks_per_visit_percentile
from freq f