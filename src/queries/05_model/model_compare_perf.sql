select b.id,
    ...
    b.mean_accuracy
from (
        select sqrt(order_by_mean_fit_time) * sqrt(order_by_mean_roc_auc) comb_order,
            *
        from (
                select r.id,
                    row_number() over (
                        order by s.mean_test_roc_auc desc
                    ) order_by_mean_roc_auc,
                    row_number() over (
                        order by s.mean_fit_time
                    ) order_by_mean_fit_time,
                    t.model_type,
                ...
                from model.runs r
                    ...
                    join model.types t on t.id = r.model_type_id
                where s.mean_test_roc_auc is not null
            ) a
    ) b
    join model.features f on f.run_id = b.id
order by comb_order