SELECT count(*) n_students,
    ...
    sum(cast(s.date_registration as float)) / count(*) avg_date_reg,
    sum(s.date_unregistration) sum_date_unreg,
    sum(cast(s.date_unregistration as float)) / count(*) avg_date_unreg,
    sum(s.date_unregistration - s.date_registration) sum_reg_date_dif,
    sum(
        cast(
            s.date_unregistration - s.date_registration as float
        )
    ) / count(*) avg_reg_date_dif,
    sum(s.studied_credits) sum_studied_credits,
    sum(cast(s.studied_credits as float)) / count(*) avg_studied_credits,
    sum(s.num_of_prev_attempts) sum_prev_attempts,
    sum(cast(s.num_of_prev_attempts as float)) / count(*) avg_prev_attempts,
    sum(
        case
    ...
    sum(
        case
            when s.is_female = '1' then 1.00000000
            else 0.00000000
        end
    ) / count(*) gender_ratio,
    sum(
    ...
    sum(
        case
            when s.has_disability = '1' then 1.000000000
            else 0.000000000
        end
