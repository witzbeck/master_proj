...
case
        when stddev_n_activity_types = 0 then 0
            ...
            sum((v.n_activity_types - a.avg_n_activity_types) ^ 3) / a.n
        ) / (stddev_n_activity_types ^ 3)
    end skew_n_activity_types,
case
        when stddev_n_activity_types = 0 then 0
        else (
            sum((v.n_activity_types - a.avg_n_activity_types) ^ 4) / a.n
...
case
    ...
    end fp_coeff into agg.mom_1234_by_interaction
from agg.v_student_activities_per_day v
    join agg.student_clicks_per_date c on c.student_id = v.student_id
    and c.date = v.date
    join act a on a.student_id = v.student_id --where stddev_clicks <> 0
    --and stddev_date <> 0
