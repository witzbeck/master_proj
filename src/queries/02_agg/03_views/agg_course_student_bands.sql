...
into agg.course_student_bands_results
from main.student_info s

join main.v_regions_onehot vro on vro.id=s.region_id
join main.v_age_band_onehot vabo on vabo.id=s.age_band_id
join main.v_highest_education_onehot vh on vh.id=s.highest_education_id
join main.v_imd_band_onehot vibo on vibo.id=s.imd_band_id

group by s.course_id, s.final_result_id